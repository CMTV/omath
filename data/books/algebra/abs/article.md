---
{
    ...
    "tags": ["величина числа", "модуль", "абсолютная величина", "неравенство треугольника", "расстояние между точками", "окрестность", "уравнения с модулем", "неравенства с модулем"]
}
---
...
Более строгим подходом является определение модуля как функции. В этом случае модуль спускается с небес на змелю и теряет свой статус "неотъемлемой части любого числа", но зато у нас появляется возможность использовать модуль в большом и наработанном аппарате математики (и математического анализа):
...
![График функции модуля](abs-plot.svg){: .w8 }
...
<theorem title="Свойства модуля как функции">
    <statement>
        <ul>
            ...
            <li>Монотонно убывает на $(-\infty, 0]$ и возрастает на $[0, +\infty)$</li>
        </ul>
    </statement>
</theorem>
...
<important>
    ...
    Поэтому, с этого момента мы тоже будем придерживаться этой логики.
    Кроме того, в дальнейших разделах вместо записей $|x|$ и $|y|$ я буду использовать записи $|f|$ и $|g|$, чтобы подчеркнуть, что под знаком модуля может быть как число, так и какая-нибудь функция.
</important>

## Уравнения с модулем {: #abs-equation }

Научимся решать уравнения, в которых присутсвует модуль.
...
<example>
    ...
    <solution>
        Определим, при каких $a$ решений у уравнения не будет. Решений у него не будет, когда его правая часть отрицательная, то есть при $a < 8$. Для $a\geq 8$ исходное уравнение разбивается на два:
...
![Геометрический смысл неравенств с >](greater.svg)
